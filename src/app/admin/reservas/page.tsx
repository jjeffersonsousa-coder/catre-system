...
import { Plus, Search, Eye, Check, X, Clock, ChevronDown } from 'lucide-react'
...
export default function ReservasPage() {
  ...
  async function atualizarStatus(id: string, status: string) {
    setSalvando(true)
    await sb.from('reservas').update({ status, observacao_interna: obsInterna || null }).eq('id', id)
    setSalvando(false)
    setSelecionada(null)
    carregar()
  }
  ...
  const filtradas = reservas.filter(r => {
    ...
    if (busca) {
      ...
      return r.nome.toLowerCase().includes(q) || r.igreja.toLowerCase().includes(q) || r.tipo_evento.toLowerCase().includes(q)
    }
    return true
  })
    ...
    <div className="p-6 max-w-7xl mx-auto w-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: '#6B7280' }}>Gerencie todas as solicitações do site e manuais</p>
        </div>
        <button
          onClick={() => { /* modal nova reserva */ }}
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {Object.entries({ todos: 'Todas', pendente: 'Pendentes', confirmada: 'Confirmadas', cancelada: 'Canceladas' }).map(([k, label]) => (
          <button
            key={k}
            onClick={() => setFiltro(k)}
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all"
            style={{
              background: filtro === k ? '#006494' : 'white',
              color: filtro === k ? 'white' : '#374151',
              border: filtro === k ? 'none' : '1px solid #E5E7EB',
            }}
          >
            {label}
            <span
              className="text-xs px-1.5 py-0.5 rounded-full"
              style={{ background: filtro === k ? 'rgba(255,255,255,0.25)' : '#F3F4F6' }}
            >
              {contadores[k as keyof typeof contadores]}
            </span>
          </button>
        ))}
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: '1px solid #F3F4F6' }}>
                {['Solicitante', 'Evento', 'Período', 'Hóspedes', 'Status', 'Ações'].map(h => (
                  <th key={h} className="text-left text-xs font-bold uppercase tracking-wider px-5 py-3.5" style={{ color: '#9CA3AF' }}>
                    {h}
                  </th>
                ))}
              </tr>
                ...
                return (
                  <tr key={r.id} className="hover:bg-gray-50 transition-colors" style={{ borderBottom: '1px solid #F9FAFB' }}>
                    ...
                    <td className="px-5 py-4">
                      ...
                      <div className="text-xs mt-0.5" style={{ color: '#9CA3AF' }}>{diasEntre(r.data_inicio, r.data_fim)} noites</div>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <button
                        onClick={() => { setSelecionada(r); setObsInterna(r.observacao_interna ?? '') }}
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-3">
                <span
                  ...
                  style={{ background: statusConfig[selecionada.status]?.bg, color: statusConfig[selecionada.status]?.color }}
                >
                  {statusConfig[selecionada.status]?.label}
                </span>
                <span className="text-xs" style={{ color: '#9CA3AF' }}>
                  Recebido em {new Date(selecionada.created_at).toLocaleDateString('pt-BR')}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  { l: 'Nome', v: selecionada.nome },
                  { l: 'Igreja / Org.', v: selecionada.igreja },
                  { l: 'E-mail', v: selecionada.email },
                  { l: 'Telefone', v: selecionada.telefone },
                  { l: 'Tipo de Evento', v: selecionada.tipo_evento },
                  { l: 'Hóspedes', v: String(selecionada.hospedes) },
                  { l: 'Chegada', v: fmt(selecionada.data_inicio) },
                  { l: 'Saída', v: fmt(selecionada.data_fim) },
                ].map(({ l, v }) => (
                  <div key={l}>
              ...
              {selecionada.refeicoes && (
                <div className="text-sm px-3 py-2 rounded-lg" style={{ background: '#F0FDF4', color: '#059669' }}>
                  ✓ Inclui serviço de alimentação
                </div>
              )}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Anotação interna (não aparece para o solicitante)..."
              ...
              {selecionada.status === 'pendente' && (
                <div className="flex gap-3">
                  <button
                    onClick={() => atualizarStatus(selecionada.id, 'confirmada')}
                    disabled={salvando}
                    className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-white font-semibold text-sm transition-all hover:opacity-90 disabled:opacity-60"
                    style={{ background: '#059669' }}
                  >
                    <Check size={16} /> Confirmar
                  </button>
                  <button
                    onClick={() => atualizarStatus(selecionada.id, 'cancelada')}
                    disabled={salvando}
                    className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl font-semibold text-sm transition-all hover:opacity-90 disabled:opacity-60 border-2"
              ...
              {selecionada.status === 'confirmada' && (
                <div className="flex gap-3">
                  <button
                    onClick={() => atualizarStatus(selecionada.id, 'concluida')}
                    disabled={salvando}
                  ...
                  >
                    <Clock size={16} /> Marcar como Concluída
                  </button>
                  <button
                    onClick={() => atualizarStatus(selecionada.id, 'cancelada')}
                    disabled={salvando}
              ...
              {(selecionada.status === 'cancelada' || selecionada.status === 'concluida') && (
                <button
                  onClick={() => atualizarStatus(selecionada.id, 'pendente')}
                  disabled={salvando}
